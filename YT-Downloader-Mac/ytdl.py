...
FFMPEG_PATH = os.path.join(BASE_DIR, "ffmpeg", "ffmpeg")  # Путь к ffmpeg в папке ffmpeg
...
def check_ffmpeg():
    ...
    if not os.path.isfile(FFMPEG_PATH):
        raise FileNotFoundError("FFmpeg not found in the specified directory. Please ensure FFmpeg is in the 'ffmpeg' folder.")
    try:
        result = os.system(f"{FFMPEG_PATH} -version")
        if result != 0:
            raise FileNotFoundError("FFmpeg not found. Please ensure FFmpeg is installed.")
    except Exception as e:
        raise FileNotFoundError(f"FFmpeg not found. Error: {str(e)}")
...
if __name__ == '__main__':
    app.run(debug=False, host='0.0.0.0', port=8215)
